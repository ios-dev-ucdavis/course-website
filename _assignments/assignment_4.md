...
In this sub-stage, you need to go over the official document on `Picker` from [here](https://developer.apple.com/documentation/swiftui/picker). You can just focus on all code snippets in that document to have a general idea of how to use the `Picker`. After that, you need to create an `enum` which will be used in our picker later. 
...
2. `.capitalized` is already defined on every `String` type variables in the standard library. You can use this handy tool to get the capitalized string.
3. `CaseIterable` and `enum` are covered in the previous lecture when we went over the Swift Type Systems.

**Grade Breakdown:**

* (5 pt) Correctly create the `enum` with right cases syntax in the right file.
...
4. Depends on different initializers you may use, the title for the `Picker` doesn't matter as well.
...
2. Go to the file `Views/InspirationViews/InspirationView`, find the right place to add a `switch` statement to conditionally display `QuoteView` or `InspirationView` based on users' choice. **You must use `switch` statement here.**
...
In this sub-stage, you will get yourself familar with the API service we will use for this assignment. We will use the API service provided by [ZenQuotes.io](https://zenquotes.io/).
...
Based on this pattern and structure, create a file named `Quote.swift` in the folder `Models`. Inside the newly created `Quote.swift` file, create a struct named `Quote` to represent the each individual "quote".

**There are some requirements:**
1. You are responsible to create right fields with correct name in your `Quote` struct based on the payload you observe.
...
3. You are responsible to conform this struct to the right protocol, which makes the struct can be decoded from a JSON string.
4. You might need to one or more fields to the `Optional` type, as the payload might not always return all fields that are shown in the official document. However, we will not use these fields that are not returned in the payload. Therefore, it's ok if you just not declare them in the code at all.
...
2. You must create a class in that newly created `QuoteAPIService.swift` file. Your class should be named as `QuoteAPIService`.
...
5. You should handle errors in the function `getQuotes()`. If error occurs, simply return `nil`.
6. You must check HTTP response code. Namely, you should check whether the response code is 200, which indicates a successfully action. If it's not, again, return `nil`.
7. You must use `try-do-catch` block on `URLSession.shared.data()` to catch and print the error if error occurs.
8. You must use `try?` when calling `JSONDecoder().decode()`.
9. The last two requirements just to ensure you know how to apply different styles of error handling on your own.
...
4. In this `QuoteViewModel`, you must have a field - `quotes: [Quote]`. We will fetch the `Quote`s via API request and assign to this variable later. We will also use this variable to display `Quote`s in our `QuoteView` later. Therefore, you must mark this field with right property wrapper.
5. You must provide a function named `loadQuotes()`.
   ...
   In this function, you need to call the `getQuotes()` function that we just defined in the previous stage in the class `QuoteAPIService`. If the returned data from the `getQuotes()` function is not `nil`, you need to assign the returned data to the `quotes` variable, which is defined in the step 4. Otherwise, just do nothing.
6. For the step 5, you must ensure the data assignment happens in the right `Actor`.
7. You are encouraged to check whether the Task is cancelled and early terminate the execution in the `loadQuotes()` function. This is a good pratice.
...
**There are some steps and requirements to follow:**
1. Declare a local instance of `QuoteViewModel` marked with right proper wrapper. `@ObservedObject` or `@StateObject`?
...
4. The quote and author text must both be selectable by long pressing. See the hint section. You don't need to set up the share functionality when do a long press.
...
2. You can use `.multilineTextAlignment()` to adjust the multiline alignment behavior for the `Text` if you need.

**Grade Breakdown:**

* (5 pt) Correctly create the `QuoteView` and have the similar layout for each quote and similar behavior as expected.
